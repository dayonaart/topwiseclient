...
class PbocParameter implements Serializable, EmvParameter {
    ...
    public String pack() throws EmvParameterException {
        ...
        tlvList.addTLV(EMVTag.C_TAG_TM_9F7A, new byte[]{(byte) (this.isSupportECash ? 1 : 0)});

        tlvList.addTLV(EMVTag.C_TAG_TM_DF69, new byte[]{(byte) (this.isSupportSm ? 1 : 0)});
        ...
        tlvList.addTLV(EMVTag.C_TAG_TM_9F7B, BytesUtil.toBCDAmountBytes(this.ecLimit));
        ...
        tlvList.addTLV(EMVTag.C_TAG_TM_TRANS_LIMIT, BytesUtil.toBCDAmountBytes(this.rfTransactionLimit));


        if (this.rfCvmLimit == null || this.rfCvmLimit < 0) {
            throw new EmvParameterException("Amount should be an integer greater than or equal to 0.");
        }
        tlvList.addTLV(EMVTag.C_TAG_TM_CVM_LIMIT, BytesUtil.toBCDAmountBytes(this.rfCvmLimit));

        if (this.rfFloorLimit == null || this.rfFloorLimit < 0) {
            throw new EmvParameterException("Amount should be an integer greater than or equal to 0.");
        }
        tlvList.addTLV(EMVTag.C_TAG_TM_FLOOR_LIMIT, BytesUtil.toBCDAmountBytes(this.rfFloorLimit));
        ...
        tlvList.addTLV(EMVTag.C_TAG_TM_9F66, this.transactionProperties);
