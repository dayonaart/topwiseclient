...
class MasterParameter implements Serializable, EmvParameter {
    ...
    public String pack() throws EmvParameterException {
        ...
        tlvList.addTLV(EMVTag.DEF_TAG_M_TRANS_MODE, new byte[]{this.mode.byteValue()});

        if (this.balanceFlag == null) {
            throw new EmvParameterException("Please set balance flag.");
        }
        tlvList.addTLV(EMVTag.DEF_TAG_M_BALANCE_SUP, new byte[]{this.balanceFlag.byteValue()});

        if (this.supportRecovery == null) {
            throw new EmvParameterException("Please set recovery flag: support recovery or not.");
        }
        tlvList.addTLV(EMVTag.DEF_TAG_M_TORN_TRANS, new byte[]{(byte) (this.supportRecovery ? 1 : 0)});

        if (this.supportCdv == null) {
            throw new EmvParameterException("Please set CDV flag.");
        }
        tlvList.addTLV(EMVTag.DEF_TAG_M_CDV_SUP, new byte[]{(byte) (this.supportCdv ? 1 : 0)});
        ...
        tlvList.addTLV(EMVTag.M_TAG_TM_TRANS_LIMIT, BytesUtil.toBCDAmountBytes(this.rfTransactionLimit));

        if (this.rfTransactionLimitCdv == null || this.rfTransactionLimitCdv < 0) {
            throw new EmvParameterException("Amount should be a long integer greater than or equal to 0.");
        }
        tlvList.addTLV(EMVTag.M_TAG_TM_TRANS_LIMIT_CDV, BytesUtil.toBCDAmountBytes(this.rfTransactionLimitCdv));

        if (this.rfCvmLimit == null || this.rfCvmLimit < 0) {
            throw new EmvParameterException("Amount should be a long integer greater than or equal to 0.");
        }
        tlvList.addTLV(EMVTag.M_TAG_TM_CVM_LIMIT, BytesUtil.toBCDAmountBytes(this.rfCvmLimit));

        if (this.rfFloorLimit == null || this.rfFloorLimit < 0) {
            throw new EmvParameterException("Amount should be a long integer greater than or equal to 0.");
        }
        tlvList.addTLV(EMVTag.M_TAG_TM_FLOOR_LIMIT, BytesUtil.toBCDAmountBytes(this.rfFloorLimit));

        if (this.cvmCapReq == null) {
            throw new EmvParameterException("Please set valid CVM capability.");
        }
        tlvList.addTLV(EMVTag.DEF_TAG_M_REQ_CVM, new byte[]{this.cvmCapReq});
        ...
        tlvList.addTLV(EMVTag.DEF_TAG_M_REQ_NOCVM, new byte[]{this.cvmCapNoReq});
        ...
        tlvList.addTLV(EMVTag.DEF_TAG_M_MAG_REQ_CVM, new byte[]{this.cvmCapMagReq});
        ...
        tlvList.addTLV(EMVTag.DEF_TAG_M_MAG_REQ_NOCVM, new byte[]{this.cvmCapMagNoReq});

        if (this.transactionCurrencyExponent == null) {
            throw new EmvParameterException("Please set transaction currency exponent.");
        }
        tlvList.addTLV(EMVTag.EMV_TAG_TM_CUREXP, new byte[]{this.transactionCurrencyExponent});
        ...
        tlvList.addTLV(EMVTag.M_TAG_TM_9F7C, this.merchantCustomData);

        if (this.transactionCategoryCode == null) {
            throw new EmvParameterException("Merchant custom data should be a buffer of length 20.");
        }
        tlvList.addTLV(EMVTag.M_TAG_TM_9F53, new byte[]{this.transactionCategoryCode});
        ...
        tlvList.addTLV(EMVTag.M_TAG_TM_9F6D, this.magCardModeVersion);
