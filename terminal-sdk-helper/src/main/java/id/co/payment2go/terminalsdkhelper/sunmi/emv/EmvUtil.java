package id.co.payment2go.terminalsdkhelper.sunmi.emv;

import com.sunmi.pay.hardware.aidlv2.bean.AidV2;
import com.sunmi.pay.hardware.aidlv2.bean.CapkV2;
import com.sunmi.pay.hardware.aidlv2.bean.EmvTermParamV2;
import com.sunmi.pay.hardware.aidlv2.emv.EMVOptV2;

import java.util.HashMap;
import java.util.Map;

import id.co.payment2go.terminalsdkhelper.common.model.BytesUtil;

public final class EmvUtil {
    public static final String COUNTRY_INDONESIA = "Indonesia";

    private EmvUtil() {
    }

    /**
     * Config country code,capability,transaction currency code,transaction currency code exponent
     * <br/> Client should config their own data according their country.
     */
    public static Map<String, String> getConfig(String country) {
        Map<String, String> map = new HashMap<>();
        switch (country) {
            case COUNTRY_INDONESIA:
            default:
                map.put("countryCode", "0360");     // country code(国家代码)
                map.put("capability", "E0F8C8");    // capability(终端性能)
                map.put("5F2A", "0360");            // transaction currency code(交易货币代码)
                map.put("5F36", "00");              // transaction currency code exponent(交易货币代码指数)
        }
        return map;
    }

    /**
     * Initialize keys.
     * <br/>Client App should save their own keys.
     */

    /**
     * Initialize AIDs and RIDs.
     * <br/>Client App should save their own AIDs and RIDs
     * <br/> Note: The AIDs and RIDs shown in this method may not
     * useful for your App, please save your own AIDs and RIDs at
     * this step.
     * <br/> SDK have several built-in AIDs and Capks, if dont want them,
     * call EMVOptV2.deleteAid(null) or EMVOptV2.deleteCapk(null,null)
     * firstly.
     */
    public static void initAidAndRid(EMVOptV2 emvOptV2) {
        try {
//            emvOptV2.deleteAid(null);//clear SDK built-in AIDs
            // clear SDK built-in AIDs and CAPKs
            emvOptV2.deleteAid(null);
            emvOptV2.deleteCapk(null, null);
            // Normal AIDs
            AidV2 aid = EmvUtil.hexStr2Aid("9F0608A000000333010101DF0101009F08020020DF1105D84000A800DF1205D84004F800DF130500100000009F1B040000C350DF150400000000DF160199DF170199DF14039F3704DF1801019F7B06100000000000DF1906100000000000DF2006100000000000DF21061000000000005F2A020643");
            emvOptV2.addAid(aid);
            aid = EmvUtil.hexStr2Aid("9F0608A000000333010102DF0101009F08020020DF1105D84000A800DF1205D84004F800DF130500100000009F1B040000C350DF150400000000DF160199DF170199DF14039F3704DF1801019F7B06100000000000DF1906100000000000DF2006100000000000DF21061000000000005F2A020643");
            emvOptV2.addAid(aid);
            aid = EmvUtil.hexStr2Aid("9F0608A000000333010103DF0101009F08020020DF1105D84000A800DF1205D84004F800DF130500100000009F1B040000C350DF150400000000DF160199DF170199DF14039F3704DF1801019F7B06100000000000DF1906100000000000DF2006100000000000DF21061000000000005F2A020643");
            emvOptV2.addAid(aid);
            aid = EmvUtil.hexStr2Aid("9F0608A000000333010106DF0101009F08020020DF1105D84000A800DF1205D84004F800DF130500100000009F1B040000C350DF150400000000DF160199DF170199DF14039F3704DF1801019F7B06100000000000DF1906100000000000DF2006100000000000DF21061000000000005F2A020643");
            emvOptV2.addAid(aid);
            // Russia AIDs
            aid = EmvUtil.hexStr2Aid("");
            emvOptV2.addAid(aid);
            aid = EmvUtil.hexStr2Aid("9F0607A00000000310109F09020000DF11050000000000DF12050000000000DF130500000000009F1B0400000000DF150400000000DF160100DF170100DF14039F37049F3C0206439F3D0102DF1906000000000000DF2006999999999999DF21060000000000009F4E009F150211119F160c746573746D65726368616E74");
            emvOptV2.addAid(aid);
            // India AIDs
            aid = EmvUtil.hexStr2Aid("9F0607A00000052410109F09020064DF1105FFFFFFFFFFDF1205FFFFFFFFFFDF130500000000009F1B0400000000DF150400000000DF160100DF170100DF14039F37049F3C0203569F3D0102DF1906000000000000DF2006999999999999DF21060000000030009F4E009F15009F1600");
            emvOptV2.addAid(aid);
            aid = EmvUtil.hexStr2Aid("9F0607A00000015230109F09020001DF1105FFFFFFFFFFDF1205FFFFFFFFFFDF130500000000009F1B0400000000DF150400000000DF160100DF170100DF14039F37049F3C0203569F3D0102DF1906000000000000DF2006999999999999DF21060000000000009F4E009F15009F1600");
            emvOptV2.addAid(aid);
            aid = EmvUtil.hexStr2Aid("9F0607A00000006510109F09020200DF1105FFFFFFFFFFDF1205FFFFFFFFFFDF130500000000009F1B0400000000DF150400000000DF160100DF170100DF14039F37049F3C0203569F3D0102DF1906000000000000DF2006999999999999DF21060000000000009F4E009F15009F1600");
            emvOptV2.addAid(aid);
            aid = EmvUtil.hexStr2Aid("9F0608A0000003330101019F09020200DF1105FFFFFFFFFFDF1205FFFFFFFFFFDF130500000000009F1B0400000000DF150400000000DF160100DF170100DF14039F37049F3C0203569F3D0102DF1906000000000000DF2006999999999999DF21060000000000009F4E009F15009F1600");
            emvOptV2.addAid(aid);
            aid = EmvUtil.hexStr2Aid("9F0608A0000003330101029F09020200DF1105FFFFFFFFFFDF1205FFFFFFFFFFDF130500000000009F1B0400000000DF150400000000DF160100DF170100DF14039F37049F3C0203569F3D0102DF1906000000000000DF2006999999999999DF21060000000000009F4E009F15009F1600");
            emvOptV2.addAid(aid);
            aid = EmvUtil.hexStr2Aid("9F0608A0000003330101039F09020200DF1105FFFFFFFFFFDF1205FFFFFFFFFFDF130500000000009F1B0400000000DF150400000000DF160100DF170100DF14039F37049F3C0203569F3D0102DF1906000000000000DF2006999999999999DF21060000000000009F4E009F15009F1600");
            emvOptV2.addAid(aid);
            //AE
            aid = EmvUtil.hexStr2Aid("");
            emvOptV2.addAid(aid);
            //添加 MCCS的AID
            aid = hexStr2Aid("");
            emvOptV2.addAid(aid);
            //添加 JCB的AID
            aid = EmvUtil.hexStr2Aid("9F0605A0000000659F09020200DF1105FFFFFFFFFFDF1205FFFFFFFFFFDF130500000000009F1B0400000000DF150400000000DF160100DF170100DF14039F37049F3C0203569F3D0102DF1906000000000000DF2006999999999999DF21060000000000009F4E009F15009F1600");
            emvOptV2.addAid(aid);
            //添加 JCB的AID
            aid = EmvUtil.hexStr2Aid("9F0607A00000006510109F09020200DF1105FFFFFFFFFFDF1205FFFFFFFFFFDF130500000000009F1B0400000000DF150400000000DF160100DF170100DF14039F37049F3C0203569F3D0102DF1906000000000000DF2006999999999999DF21060000000000009F4E009F15009F1600");
            emvOptV2.addAid(aid);
            // GPN AID
            aid = EmvUtil.hexStr2Aid("9F0607A00000060210109F09020200DF1105FFFFFFFFFFDF1205FFFFFFFFFFDF130500000000009F1B0400000000DF150400000000DF160100DF170100DF14039F37049F3C0203569F3D0102DF1906000000000000DF2006999999999999DF21060000000000009F4E009F15009F1600");
            emvOptV2.addAid(aid);

            // Normal RIDs
            CapkV2 capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            // Russia RIDs
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            // India RIDs
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            //AE Capk
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            //JCB Capk
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
            capkV2 = EmvUtil.hexStr2Rid("");
            emvOptV2.addCapk(capkV2);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    /**
     * Initialize NFC-Speedup aid and rid
     * <p>
     * /**
     * Set EMV terminal parameters.
     * <br/> Client app should save their own terminal parameters
     */
    public static void setTerminalParam(Map<String, String> map, EMVOptV2 emvOptV2) {
        try {

            EmvTermParamV2 emvTermParam = new EmvTermParamV2();
            emvTermParam.countryCode = map.get("countryCode");
            emvTermParam.capability = map.get("capability");
            emvTermParam.currencyCode = map.get("countryCode");
//            emvTermParam.currencyCode = map.get("5F2A");
//            emvTermParam.currencyExp = map.get("5F36");
            int result = emvOptV2.setTerminalParam(emvTermParam);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    /**
     * Convert Hex string AID to AidV2
     */
    public static AidV2 hexStr2Aid(String hexStr) {
        AidV2 aidV2 = new AidV2();
        Map<String, TLV> map = TLVUtil.buildTLVMap(hexStr);
        TLV tlv = map.get("DF21");
        if (tlv != null) {
            aidV2.cvmLmt = BytesUtil.hexString2Bytes(tlv.getValue());
        }
        tlv = map.get("DF20");
        if (tlv != null) {
            aidV2.termClssLmt = BytesUtil.hexString2Bytes(tlv.getValue());
        }
        tlv = map.get("DF19");
        if (tlv != null) {
            aidV2.termClssOfflineFloorLmt = BytesUtil.hexString2Bytes(tlv.getValue());
        }
        tlv = map.get("9F7B");
        if (tlv != null) {
            aidV2.termOfflineFloorLmt = BytesUtil.hexString2Bytes(tlv.getValue());
        }
        tlv = map.get("9F06");
        if (tlv != null) {
            aidV2.aid = BytesUtil.hexString2Bytes(tlv.getValue());
        }
        tlv = map.get("DF01");
        if (tlv != null) {
            aidV2.selFlag = BytesUtil.hexStr2Byte(tlv.getValue());
        }
        tlv = map.get("DF17");
        if (tlv != null) {
            aidV2.targetPer = BytesUtil.hexStr2Byte(tlv.getValue());
        }
        tlv = map.get("DF16");
        if (tlv != null) {
            aidV2.maxTargetPer = BytesUtil.hexStr2Byte(tlv.getValue());
        }
        tlv = map.get("9F1B");
        if (tlv != null) {
            aidV2.floorLimit = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("DF15");
        if (tlv != null) {
            aidV2.threshold = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("DF13");
        if (tlv != null) {
            aidV2.TACDenial = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("DF12");
        if (tlv != null) {
            aidV2.TACOnline = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("DF11");
        if (tlv != null) {
            aidV2.TACDefault = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("9F01");
        if (tlv != null) {
            aidV2.AcquierId = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("DF14");
        if (tlv != null) {
            aidV2.dDOL = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("9F09");
        if (tlv != null) {
            aidV2.version = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("9F4E");
        if (tlv != null) {
            aidV2.merchName = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("9F15");
        if (tlv != null) {
            aidV2.merchCateCode = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("9F16");
        if (tlv != null) {
            aidV2.merchId = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("9F3C");
        if (tlv != null) {
            aidV2.referCurrCode = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("9F3D");
        if (tlv != null) {
            aidV2.referCurrExp = BytesUtil.hexStr2Byte(tlv.getValue());
        }
        tlv = map.get("DFC108");
        if (tlv != null) {
            aidV2.clsStatusCheck = BytesUtil.hexStr2Byte(tlv.getValue());
        }
        tlv = map.get("DFC109");
        if (tlv != null) {
            aidV2.zeroCheck = BytesUtil.hexStr2Byte(tlv.getValue());
        }
        tlv = map.get("DFC10A");
        if (tlv != null) {
            aidV2.kernelType = BytesUtil.hexStr2Byte(tlv.getValue());
        }
        tlv = map.get("DFC10B");
        if (tlv != null) {
            aidV2.paramType = BytesUtil.hexStr2Byte(tlv.getValue());
        }
        tlv = map.get("9F66");
        if (tlv != null) {
            aidV2.ttq = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("9F1C");
        if (tlv != null) {
            aidV2.termId = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("9F1D");
        if (tlv != null) {
            aidV2.riskManData = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("DF8101");
        if (tlv != null) {
            aidV2.referCurrCon = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("DF8102");
        if (tlv != null) {
            aidV2.tDOL = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("DFC10C");
        if (tlv != null) {
            aidV2.kernelID = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        return aidV2;
    }

    /**
     * Convert Hex string RID to CapkV2
     */
    public static CapkV2 hexStr2Rid(String hexStr) {
        CapkV2 capkV2 = new CapkV2();
        Map<String, TLV> map = TLVUtil.buildTLVMap(hexStr);
        TLV tlv = map.get("9F06");
        if (tlv != null) {
            capkV2.rid = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("9F22");
        if (tlv != null) {
            capkV2.index = BytesUtil.hexStr2Byte(tlv.getValue());
        }
        tlv = map.get("DF06");
        if (tlv != null) {
            capkV2.hashInd = BytesUtil.hexStr2Byte(tlv.getValue());
        }
        tlv = map.get("DF07");
        if (tlv != null) {
            capkV2.arithInd = BytesUtil.hexStr2Byte(tlv.getValue());
        }
        tlv = map.get("DF02");
        if (tlv != null) {
            capkV2.modul = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("DF04");
        if (tlv != null) {
            capkV2.exponent = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("DF05");
        if (tlv != null) {
            capkV2.expDate = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        tlv = map.get("DF03");
        if (tlv != null) {
            capkV2.checkSum = BytesUtil.hexStr2Bytes(tlv.getValue());
        }
        return capkV2;
    }


}
